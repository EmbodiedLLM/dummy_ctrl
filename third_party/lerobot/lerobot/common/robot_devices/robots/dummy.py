...
from lerobot.common.robot_devices.teleop.dummy_ctrl import DummyCtrl
class DummyRobot:
    def __init__(self, config: DummyRobotConfig | None = None, **kwargs):
        ...
        self.leader_arm_motors = make_motors_buses_from_configs(self.config.leader_arm)
        self.leader_arm = self.leader_arm_motors['main']
        self.follower_arm_motors = make_motors_buses_from_configs(self.config.follower_arm)
        self.follower_arm = self.follower_arm_motors['main']
    ...
    def motor_features(self) -> dict:
        action_names = get_motor_names(self.leader_arm_motors)
        state_names = get_motor_names(self.follower_arm_motors)
        return {
            "action": {
                "dtype": "float32",
    ...
    def teleop_step(
        ...
        if self.teleop is None and self.inference_time:
            self.teleop = DummyCtrl()

        # read target pose state as 
        before_read_t = time.perf_counter()
        state = self.follower_arm.read() # read current joint position from robot
